...
const TieredUpgrades = (data: ICreateTieredUpgradeDTO[]): void => {
    data.forEach(upgradeData => {
        ...
        // @ts-ignore
        const desc =
            (upgradeData.description || upgradeData.building.replace(/y$/, 'ie') + tierDesc) +
            quote;
        ...
        if (upgradeData.icon) {
            ...
            icon = [
                buildingsList.indexOf(upgradeData.building) + 3,
                tierRows[upgradeData.tier],
                upgradeIconsUrl,
            ];
        }
        ...
        if (upgradeData.building) {
            if (upgradeData.building === 'Cursor') {
                ...
                upgrade.order =
                    Math.floor(Game.Objects[upgradeData.building].tieredUpgrades[1].order) + 0.9999;
            }
        } else {
