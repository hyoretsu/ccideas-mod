export const misfortunePrice = 6.66e28;
...
const customMods = (): void => {
    ...
    Object.assign(Game.Tiers, {
        ...
        raingrid: new Tier('Raingrid', '#a000d5', 7),
        misfortune: new Tier('Misfortune', '#7e5a40', 0, misfortunePrice),
    });

    injects.cursorCps.push("if(Game.Has('Raingrid fingers')){add*=50;}");
    // Misfortune tier injections
    injects.cpsMult.push(
        "if(Game.Has('Misfortune #600')){mult*=1.01};if(Game.Has('Misfortune #601')){mult*=1.06}",
    );
    injects.kittenUpgrade.push(
        "if(Game.Has('Misfortune #603')){catMult*=1+Game.milkProgress*0.05*milkMult}",
    );
    injects.getTieredCpsMult.push(
        'if(me.vanilla===1&&me.tieredUpgrades.misfortune&&Game.Has(me.tieredUpgrades.misfortune.name)){switch(Game.elderWrath){case 1:mult*=1.02;break;case 2:mult*=1.04;break;case 3:mult*=1.06;break;}}',
    );
    injects.modifyBuildingPrice.push(
        "if(building.vanilla===1&&building.tieredUpgrades.misfortune&&Game.Has(building.tieredUpgrades.misfortune.name)){switch(Game.elderWrath){case 1:price*=0.98;break;case 2:price*=0.96;break;case 3:price*=0.94;break;}}if(Game.Has('Misfortune #600')){price*=0.99;}",
    );
